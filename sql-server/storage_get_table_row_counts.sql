...
DESCRIPTION: This script is based on the disk and TABLE reports available WITH SSMS for 
a SQL Server database.  I captured the SQL Profiler activity while running those reports
and modified them to suit most people's needs.  I also modified the code so that it will
...
WHILE @database_pos < @database_count
	BEGIN		
		...
		FROM [' + @database_name + '].sys.tables t WITH (nolock)
		INNER JOIN [' + @database_name + '].sys.schemas s WITH (NOLOCK) 
			...
			BEGIN	
				BEGIN TRY			
					...
					FROM (SELECT [object_id], index_id 
						FROM [' + @database_name + '].sys.indexes i WITH (NOLOCK) 
...
SELECT [database_name], [schema_name], table_name, [row_count_disp] = FORMAT([row_count], '#,##0'), [get_count_failed], [table_create_date]
